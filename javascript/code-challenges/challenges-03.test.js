...
const typeNum = (arr) => {
  let newArray = [];
  newArray = arr.filter(element => typeof element === 'number'
  );
  return newArray;
};
...
const containsAnd = (arr) => {
  let newArray = [];
  let letter = 'and';
  newArray = arr.filter(element => element.includes(letter)
  );
  return newArray;
};
...
const oddValues = (arr) => {
  ...
  newArray = arr.filter(element => element % 2 === 1);
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  rbiddenValues = arr.filter(element => forbiddenValues.indexOf(element)===-1);

  return rbiddenValues ;
};
